...
class Territory_gui(Plugin):
    ...
    def on_enable(self) -> None:
        ...
        try:
            ...
            self.server.logger.info(f"Territory插件版本{tty_version},图形菜单版本{tty_gui_version}")
        except:
            self.server.logger.error("未检测到Territory插件本体,插件拒绝加载")
    ...
    commands = {
        "ttygui": {
            "description": "领地菜单命令--用法:",
            "usages": [
                "/ttygui (list)<opt: opt_list>"
                ],
    ...
    def on_command(self, sender: CommandSender, command: Command, args: list[str]) -> bool:
        ...
        if command.name == "ttygui":
            if not isinstance(sender,Player):
                self.server.logger.error("无法在控制台使用此命令")
                return
            if len(args) == 0:
                # 图形界面
                            ...
                            def man_tty_sub_menu(sender,json_str:str):
                                ...
                                def mtsm_on_click(sender):
                                    mtsm_form = ModalForm(
                                        ...
                                        controls=[
                                            Toggle(label="§l是否允许外人领地内交互",default_value=if_jiaohu_status),
                                            Toggle(label="§l是否允许外人领地内破坏",default_value=if_break_status),
                                            Toggle(label="§l是否允许外人传送至领地",default_value=if_tp_status),
                                            Toggle(label="§l是否允许外人领地内放置",default_value=if_build_status),
                                            Toggle(label="§l是否允许领地内实体爆炸",default_value=if_bomb_status),
                                            Toggle(label="§l是否允许外人对实体攻击",default_value=if_damage_status),
                                        ],
                                        on_submit=run_mtsm,
                ...
                list_tty_button = ActionForm.Button(text="§l§5列出自己的全部领地",icon="textures/ui/infobulb",on_click=run_list())
                # 发送菜单
                form = ActionForm(
