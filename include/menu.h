...
public:
  ...
  void openTpTtyMenu(endstone::Player* player) const {
    // 获取玩家成员及以上权限的领地列表
    auto ttyList = Territory_Action::getMemberTtyNames(player->getName());
    if (ttyList.empty()) {
  ...
  void openAddTtyMemberSubMenu(endstone::Player* player, const Territory_Action::TerritoryData& tty) const {
    auto onlinePlayers = getOnlinePlayerList();
    endstone::ModalForm form;
    ...
    form.setOnSubmit([=](const endstone::Player* p, const std::string& response) {
      try {
        auto parsed = nlohmann::json::parse(response);
        int idx = parsed[0].get<int>();
        std::string leave_player = parsed[1];
